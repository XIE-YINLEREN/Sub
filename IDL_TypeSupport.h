#ifndef _IDL_TYPESUPPORT_H_INCLUDED_ 
#define _IDL_TYPESUPPORT_H_INCLUDED_
#include <ctime> 
#include <string>
using std::string;
#include "IDL_DataWriter.h"
#include "IDL_DataReader.h"
#include "include/DDS/TypeSupport.h"
#include "MD5.h"
#include "include/DDS/DomainParticipant.h"
class DomainParticipant;

list<string> ListData;
static void initList()
{
	if (ListData.size() <= 0)
	{
		ListData.push_back("struct UserDataType{");
		ListData.push_back("short x;");
		ListData.push_back("long y;");
		ListData.push_back("string 255 color;");
		ListData.push_back("};");
	}
}

class UserDataTypeTypeSupport : public TypeSupport {
public:
	static UserDataType* create_data()
	{
		return new UserDataType;
	}

	static ReturnCode_t delete_data(UserDataType* pUserDataType)
	{
		delete pUserDataType;
		pUserDataType = NULL;
		return ReturnCode_t();
	}

	static int getTime()  //获取时间
	{
		return clock() / CLOCKS_PER_SEC;
	}


	static void print_data(UserDataType* pUserDataType, int layer = 0)
	{
		static int count;
		static int pre_count;
		int i = 0;


		int now = getTime();
		static int lastTime = 0;
		count++;
		if (lastTime == 0)
		{
			pUserDataType->num = 0;
		}

		string MD5_data = GetDataMD5(pUserDataType->data);//计算MD5
		
		if (now - lastTime > 0)//一秒钟执行一次
		{
			cout << "----------------------------------------------------------" << endl;
			cout << "UserDataType:" << std::endl;
			cout << "data = " << pUserDataType->data << endl;
			cout << "MD5 = " << pUserDataType->color << endl;
			cout << "Quantity received = " << count << endl;
			if (MD5_data != pUserDataType->color)//MD5不一致时输出错误信息
			{
				cout << "MD5 erro" << endl;
			}
			else
			{
				cout << "MD5 correct" << endl;
			}
			double rate = static_cast<double>(pUserDataType->num) / static_cast<double>(count - pre_count) * 100;
			cout << "lost rate = " << rate << "%" << endl;
			lastTime = now;
			pre_count = count;
		}
	}

	static const char* get_type_name()
	{
		return "UserDataType";
	}

	static int register_type(DomainParticipant* participant, string type_name)
	{
		if (type_name.empty())
			type_name = get_type_name();

		initList();
		return participant->register_type(type_name, ListData);
	}
};

#endif
